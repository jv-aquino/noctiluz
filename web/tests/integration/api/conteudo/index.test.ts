...
describe('GET /conteudo', () => {
  ...
  it('should return content pages for lesson', async () => {
    ...
    (conteudoService.getContentPages as Mock).mockResolvedValue([contentPageMock]);
    
    const request = createRequest({}, `conteudo?lessonId=${lessonId}`);
  ...
  it('should return empty array if no content pages exist', async () => {
    const lessonId = postLessonMock.id;
    (lessonService.getLessonById as Mock).mockResolvedValue(postLessonMock);
    (conteudoService.getContentPages as Mock).mockResolvedValue([]);
...
describe('POST /conteudo', () => {
  ...
  const lessonId = postLessonMock.id;
  const createPostRequest = () => createRequest(createContentPageMock, 'conteudo');
  ...
  it('should create content page if user is ADMIN', async () => {
    ...
    (conteudoService.createContentPage as Mock).mockResolvedValue({
      ...contentPageMock,
  ...
  it('should fail if name is missing', async () => {
    ...
    const request = createRequest({ order: 1, lessonId }, 'conteudo');
    const response = await POST(request);
  ...
  it('should use provided order if specified', async () => {
    ...
    const request = createRequest({ name: "Test Page", order: 5, lessonId }, 'conteudo');
    const response = await POST(request);
