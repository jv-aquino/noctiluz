...
// Route handlers
import { GET, POST } from '@/app/(backend)/api/contents/[pageId]/route'
import { setCurrentRole } from '../../../mocks/auth'
import { createRequest } from '../../../mocks/requests'
import { postLessonMock } from '../../../mocks/lesson'
import { contentPageMock, contentBlockMock, createContentBlockMock } from '../../../mocks/content'
...
describe('GET /api/contents/[pageId]', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should return content blocks for page', async () => {
    ...
    (contentService.getContentBlocks as Mock).mockResolvedValue([contentBlockMock]);
    ...
    expect(contentService.getContentBlocks).toHaveBeenCalledWith({ pageId });
  });
  ...
  it('should return empty array if no content blocks exist', async () => {
    ...
    (lessonService.getLessonById as Mock).mockResolvedValue(postLessonMock);
    (contentService.getContentPage as Mock).mockResolvedValue(contentPageMock);
    (contentService.getContentBlocks as Mock).mockResolvedValue([]);

    const request = createRequest({}, `content/${pageId}?lessonId=${lessonId}`);
    const response = await GET(request, {
      params: Promise.resolve({ pageId })
    });
...
describe('POST /api/content/[pageId]', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    setCurrentRole(null);
  });
  ...
  it('should create content block if user is ADMIN', async () => {
    ...
    (contentService.createContentBlock as Mock).mockResolvedValue({
      ...contentBlockMock,
