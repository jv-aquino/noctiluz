...
import { PATCH } from '@/app/(backend)/api/contents/[pageId]/order/route'
import { setCurrentRole } from '../../../../mocks/auth'
import { createRequest, returnParams } from '../../../../mocks/requests'
import * as conteudoService from '@/app/(backend)/services/content'

vi.mock('@/backend/services/content', () => ({
  reorderContentBlocks: vi.fn(),
}))

describe('PATCH /api/contents/[pageId]/order', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    setCurrentRole(null);
  });
  ...
  it('should fail if unauthenticated', async () => {
    ...
    const response = await PATCH(request, returnParams({ pageId }));
  ...
  it('should fail if blockIds are not provided', async () => {
    ...
    const response = await PATCH(request, returnParams({ lessonId, pageId }));
